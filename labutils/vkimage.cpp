...
namespace labutils
{
	Image load_image_texture2d(char const* aPath, VulkanContext const& aContext, VkCommandPool aCmdPool, Allocator const& aAllocator, VkFormat format)
	{
		...
		{
			data = stbi_load(aPath, &baseWidthi, &baseHeighti, &baseChannelsi, 1);
		}
		else if (strcmp(aPath, "") == 0) {
			data = new stbi_uc[4];
			data[0] = 128;
			data[1] = 128;
			data[2] = 255;
		...
		{
			data = stbi_load(aPath, &baseWidthi, &baseHeighti, &baseChannelsi, 4);
		}
		if (!data)
		{
		...
		// Create image
		Image ret = create_image_texture2d(aAllocator, baseWidth, baseHeight, format,
		...
		// Whole image is currently in the TRANSFER SRC OPTIMAL layout.
		image_barrier(cbuff, ret.image,
			VK_ACCESS_TRANSFER_READ_BIT,
