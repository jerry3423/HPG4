...
namespace
{
	 ...
	 */
	constexpr char kFileVariant[16] = "default-cw4";

	/* Fallback texture for RGBA 1111 and Grayscale 1
	 */
	constexpr char kTextureFallbackR1[] = "assets-src/cw4/r1.png";
	constexpr char kTextureFallbackRGBA1111[] = "assets-src/cw4/rgba1111.png";
...
#	if !defined(NDEBUG)
	 ...
	 *
	 * The binary .comp5822mesh should be unchanged between debug and release
	 * builds, so you can safely use the release build to create the file once,
	 * even while debugging the main CW4 program.
	 */
#	endif
	process_model_(
		"assets/cw4/suntemple.comp5822mesh",
		"assets-src/cw4/suntemple.obj-zstd"
	);
...
namespace
{
	...
	void write_model_data_( FILE* aOut, InputModel const& aModel, std::vector<IndexedMesh> const& aIndexedMeshes, std::unordered_map<std::string,TextureInfo_> const& aTextures )
	{
		...
		for( std::size_t i = 0; i < aModel.meshes.size(); ++i )
		{
			...
			std::vector<double> positionToDouble;
			std::vector<double> texCoordsToDouble;
			std::vector<double> normalsToDouble;
			std::vector<double> Tangents;
			std::vector<double> BiTangents;
			std::vector<double> vTangents;
			std::vector<double> vBiTangents;
			std::vector<double> Tangents4D;
			std::vector<std::size_t>IndicesTolong;
			std::vector<glm::vec4> TangentsToVec4;

			for (std::size_t j = 0; j < imesh.vert.size(); ++j)
			{
				positionToDouble.push_back(imesh.vert[j].x);
				positionToDouble.push_back(imesh.vert[j].y);
				positionToDouble.push_back(imesh.vert[j].z);
				texCoordsToDouble.push_back(imesh.text[j].x);
				texCoordsToDouble.push_back(imesh.text[j].y);
				normalsToDouble.push_back(imesh.norm[j].x);
				normalsToDouble.push_back(imesh.norm[j].y);
				normalsToDouble.push_back(imesh.norm[j].z);
			}

			for (std::size_t j = 0; j < imesh.indices.size(); ++j)
			{
				IndicesTolong.push_back(imesh.indices[j]);
			}

			tgen::computeCornerTSpace(IndicesTolong, IndicesTolong, positionToDouble, texCoordsToDouble, Tangents, BiTangents);
			tgen::computeVertexTSpace(IndicesTolong, Tangents, BiTangents, imesh.text.size(), vTangents, vBiTangents);
			tgen::orthogonalizeTSpace(normalsToDouble, vTangents, vBiTangents);
			tgen::computeTangent4D(normalsToDouble, vTangents, vBiTangents, Tangents4D);

			for (std::size_t j = 0; j < Tangents4D.size(); j += 4)
			{
				TangentsToVec4.push_back(glm::vec4(Tangents4D[j], Tangents4D[j + 1], Tangents4D[j + 2], Tangents4D[j + 3]));
			}

			checked_write_(aOut, sizeof(glm::vec4) * vertexCount, TangentsToVec4.data());
		}
	}
}
